...
try{
	if(($xt_car_src != "" || $xt_incharge_src != "") && $xt_date_src ==""){
		$sql    = "SELECT [id_rentcar]
					...
					and [incharge] like '%$xt_incharge_src%'";
	...
	elseif($xt_car_src == "" && $xt_incharge_src == "" && $xt_date_src !=""){
		$sql    = "SELECT [id_rentcar]
					...
					and id_car <> 15
					and id_car <> 16
					and datediff(day, [start_date], '$xt_date_src') = 0 or datediff(day, [end_date], '$xt_date_src') = 0 order by [start_date] desc";
		$rs     = $db_jeinid->Execute($sql);
		$return = array();
	}
	elseif(($xt_car_src != "" || $xt_incharge_src != "") && $xt_date_src !=""){
		$sql    = "SELECT [id_rentcar]
					...
					and id_car <> 15
					and id_car <> 16
					and (datediff(day, [start_date], '$xt_date_src') = 0 or datediff(day, [end_date], '$xt_date_src') = 0)
	...
	elseif($xt_car_src == "" && $xt_incharge_src == "" && $xt_date_src ==""){
		$sql    = "SELECT [id_rentcar]
					...
					and id_car <> 15
					and id_car <> 16
					and [start_date] BETWEEN convert(varchar(10), getdate(), 120) AND (SELECT MAX(convert(varchar(10), [END_DATE], 120)) FROM [JEINID].[dbo].[RENT_cars])
