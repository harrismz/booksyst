...
Ext.onReady(function(){
    ...
    //==========================DS STORE===========================================================================
        ...
        /*=========ds_detail==========*/
		/*=========ds_masteruser==========*/
        Ext.define('M_masteruser',{
            extend: 'Ext.data.Model',
            fields: ['userid']
        });
        var ds_masteruser = Ext.create('Ext.data.Store',{
            model	: 'M_masteruser',
            autoLoad: true,
            proxy	: {
                type	: 'ajax',
                api     : {
                    read    : 'json/json_user.php',
                },
                reader	: {
                    type    : 'json',
                    root	: 'data'
        ...
        /*=========ds_masteruser==========*/
       /*=========ds_uservip==========*/
        Ext.define('M_uservip',{
            extend: 'Ext.data.Model',
            fields: ['vipid','vipuser']
        });
        var ds_uservip = Ext.create('Ext.data.Store',{
            model	: 'M_uservip',
            autoLoad: true,
            proxy	: {
                type	: 'ajax',
                api     : {
                    read    : 'json/json_uservip.php',
                },
                reader	: {
                    type    : 'json',
                    root	: 'data'
    ...
    //==========================FUNCTION==============================================================================
        ...
        /*======================register_data==============================*/
        /*======================add_masteruser===========================*/
        function add_masteruser() {
            var win_addmasteruser;
            if (!win_addmasteruser) {
                var form_addmasteruser = Ext.widget('form', {
					...
					items         : [
						...
						},
						{
                            ...
                            id					: 'xt_masteruser',
                            name				: 'xt_masteruser',
                             emptyText			: 'SELECT USER VIP Room',
                            queryMode			: 'local',
                            displayField		: 'userid',
                            valueField			: 'userid',
                            store				: ds_masteruser,
                    ...
                    buttons : [
                        ...
                        },
                        {
                            ...
                            handler  	: function() {
                                ...
                                if (form.isValid()) {
                                    form.submit({
                                        url             : 'resp/resp_vip.php',
                                        waitMsg         : 'Sending your data...',
                                        submitEmptyText : false,

                                        success : function(form, action) {
                                            ds_uservip.loadPage(1);
                                            popwindow.close();
                ...
                win_addmasteruser = Ext.widget('window', {
                    title           : 'NEW USER VIP',
					width			: 300,
                    autoheight		: true,
                    modal			: false,
					...
					y				: 200,
                    layout          : 'fit',
                    animateTarget   : 'btn_addmasteruser',
                    items           : form_addmasteruser,
					bodyStyle    	: { background :'rgba(122, 0, 61, 1); padding-bottom: 5px;'},
                    listeners       : {
                            activate:function(){
                                Ext.getCmp('btn_addmasteruser').disable();
                                Ext.getCmp('btn_delmasteruser').disable();
                            },
                            close:function(){
                                Ext.getCmp('btn_addmasteruser').enable();
                                Ext.getCmp('btn_delmasteruser').enable();
                            }
                     }
                 });
            }
            win_addmasteruser.show();
        }
        /*======================add_masteruser==============================*/
        /*======================cancel_data===========================*/
        ...
        /*======================del_member===========================*/
		/*======================del_masteruser===========================*/
           function del_masteruser(widget, event) {
                var rec = Ext.getCmp("masteruser").getSelectionModel().getSelection();
                var recLength = rec.length;
                if(recLength == ""){
                    Ext.Msg.show({
                        title   : 'Warning',
                    ...
                    var win_delmasteruser;
                    if (!win_delmasteruser) {
                        var form_delmasteruser = Ext.widget('form', {
                            layout			: {
                                type	: 'vbox',
                                align	: 'stretch'
                            },
                            ...
                            items			: [
								...
								},
								{
									xtype	: 'hiddenfield',
									name	: 'xt_vipid',
									value	: rec[0].data.vipid
								},
								{
                                    xtype	: 'label',
                                    html	: 'Are you sure to DELETE this VIP User ??'
                                }
                            ],
                            buttons			: [
                                {
                        ...
                        win_delmasteruser = Ext.widget('window', {
                            title			: 'Delete Member',
							bodyStyle    	: { background :'rgba(122, 0, 61, 1); padding-bottom: 5px;'},
                            minWidth		: 380,
                            autoHeight		: true,
                            ...
                            constrain		: true,
                            layout			: 'fit',
                            animateTarget	: 'btn_delmasteruser',
                            items			: form_delmasteruser
                        });
                    }
                    win_delmasteruser.show();
                }
            }
        /*======================del_masteruser===========================*/
		/*======================view_detail========================*/
        ...
        /*======================view_detail========================*/
        /*======================masteruser========================*/
        function masteruser (widget, event){
			var win_detail;
			if (!win_detail) {
				var form_detail = Ext.widget('form',{
					layout			: {
					...
					tbar: [
						{	
							xtype  : 'button',
							id     : 'btn_delmasteruser',
							text   : 'Delete User',
							iconCls: 'icon-minus-icon8',
							tooltip: 'Cancel User Access VIP Room',
							scale  : 'medium',
							handler : del_masteruser
						},
						'->',
						{
							xtype  : 'button',
							id     : 'btn_addmasteruser',
							text   : 'Add User',
							iconCls: 'icon-plus-icon8',
							tooltip: 'Add User Access VIP Room',
							scale  : 'medium',
							handler : add_masteruser
					...
					items: [
						{
							...
							columns		: [
								...
								{ text: 'VIP ID', dataIndex: 'vipid', flex: 1, tdCls:'wrap-text', align:'left', hidden:true },
								{ text: 'VIP USER', dataIndex: 'vipuser', flex: 1, tdCls:'wrap-text', align:'left' }
							]
						}
					],
				});
				win_detail = Ext.widget('window',{
					id				: 'gridvip',
					title			: 'DETAIL VIP ACCESS',
					animateTarget	: 'btn_masteruser',
					layout			: 'fit',
					width			: 250,
					heigth			: 450,
					x				: 250,
					...
					listeners		: {
						activate:function(){
							...
							Ext.getCmp('btn_cancel').disable();
							Ext.getCmp('btn_detail').disable();
							Ext.getCmp('btn_masteruser').disable();
						},
						close:function(){
							Ext.getCmp('btn_refresh').enable();
							Ext.getCmp('btn_search').enable();
							...
							Ext.getCmp('btn_cancel').enable();
							Ext.getCmp('btn_detail').enable();
							Ext.getCmp('btn_masteruser').enable();
						}
					}
				});
				win_detail.show();
        ...
        var grid = Ext.create('Ext.grid.Panel',{
            ...
            tbar		:[{
                        ...
                        /*{
                            ...
                            iconCls: 'icon-edit-icon8',
                            scale  : 'medium',
                            tooltip: 'Editing your reservation',
                            handler: update_data
                        },*/
                        {
                            xtype  : 'button',
                        ...
                        '->',
                       {
                            xtype  : 'button',
                            id     : 'btn_masteruser',
                            text   : 'Setting User Access',
                            iconCls: 'icon-edit-icon8',
                            scale  : 'medium',
                            tooltip: 'Setting User VIP Room Access',
                            handler: masteruser
                        },
                        {
							xtype		: 'label',
