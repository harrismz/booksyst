...
try{
	...
	elseif($src_roomcode == "" && $src_date == ""){
		$sql    = "SELECT [id_rentroom],(SELECT [roomname] FROM [JEINID].[dbo].[RENT_roomdata] where [RENT_roomdata].id_room = [RENT_rooms].id_room) as roomcode
					,[date],[start_time],[end_time],[dept],[incharge],[purpose],[remark] FROM [JEINID].[dbo].[RENT_rooms] ORDER BY [date] ASC";
		$rs     = $db_jeinid->Execute($sql);
		$return = array();
	}
...
for($i=0; !$rs->EOF; $i++){
	...
	*/
	$return[$i]['date'] 	  = date_format(date_create($rs->fields['2']), 'Y-m-d');
	$return[$i]['start_time'] = date_format(date_create($rs->fields['3']), 'g:i A');
	$return[$i]['end_time']   = date_format(date_create($rs->fields['4']), 'g:i A');
